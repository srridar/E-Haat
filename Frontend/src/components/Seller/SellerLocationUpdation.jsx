...
const SellerLocationUpdation = () => {
    ...
    const fetchExistingLocation = async () => {
      try {
        ...
        if (res.data.success && res.data.data?.location) {
          ...
          console.log("this is seller data" + loc);
          const initialData = {
  ...
  const submitHandler = async () => {
    try {
      ...
      const res = await axios.put(`${SELLER_API_END_POINT}/setlocation`, payload, { withCredentials: true });
      if (res.data.success) {
...
export default SellerLocationUpdation;